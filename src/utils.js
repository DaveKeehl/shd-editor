let constants = {
	...
	get OBJECT_START_FIRST_VAR() {
		return (
			this.BLOCK_PADDING +
			this.OBJECT_HANDLE_FULL_HEIGHT +
			this.BLOCK_HEADER_HEIGHT +
			this.VAR_VERTICAL_MARGIN
		)
	}, // 101
...
function getBlockHeight(blockObject) {
	...
	result +=
		variablesCount * VAR_HEIGHT +
		(variablesCount > 0 ? BLOCK_VERTICAL_VAR_MARGIN : 0) +
		(variablesCount > 1 ? VAR_VERTICAL_MARGIN * (variablesCount - 1) : 0)
	return result
}
...
function getStackFrameVirtualData(
	stack,
	stackWidth,
	stackScrollAmount,
	mouseY
		...
		if (virtualMouseY >= startY && virtualMouseY <= endY) {
			const stackFrameVirtualData = {
				...
				coordinates: {
					...
					startY: startY,
					get endX() {
						return this.startX + getStackFrameWidth(stackWidth)
					},
					endY: endY
				}
			}
...
function getHoveredStackData(stack, stackWidth, stackScrollAmount, mouseY) {
	...
	const stackFrameVirtualData = getStackFrameVirtualData(
		stack,
		stackWidth,
		stackScrollAmount,
		mouseY
	...
	let varAccumulator =
		startY + BLOCK_PADDING + BLOCK_HEADER_HEIGHT + VAR_VERTICAL_MARGIN
...
function getStackFramePosition(stack, stackFrame, stackScrollAmount) {
	...
	for (const frame of stack) {
		...
		if (frame.id === stackFrame.id) {
			...
			console.log(`FRAME_MARGIN_BOTTOM: ${constants.FRAME_MARGIN_BOTTOM}`)
			accumulator = endY + constants.FRAME_MARGIN_BOTTOM
		}
	}
}
