...
function App() {
	...
	const {
		...
		isResizable,
		setIsResizable
	} = useContext(ResizableStackContext)
	...
	useEffect(() => {
		...
		// SEPARATOR
		const separatorWidth = parseInt(
			window.getComputedStyle(separator.current).getPropertyValue('width')
		)
			...
			if (arrows.activeDragHandle === 'start') {
				...
				const arrowStart = () => {
					...
					const heapArrowStart =
						target === undefined
							? undefined
							: arrows.getExactHeapStartPosition(
									stackWidth,
									target,
									event.clientX,
									event.clientY
					...
					if (
						stackArrowStart !== undefined &&
						heapArrowStart === undefined
					) {
				...
				if (arrowStart() !== undefined) {
					...
					app.resetVariablesValueAfterArrowDeletion(
						arrows.newArrow.from.id
					)
					arrows.setStart({
	...
	function handleMouseMove(event) {
		...
		if (isResizable && clientX >= STACK_MIN && clientX <= STACK_MAX) {
			...
			arrows.updateArrowsOnStackResize(
				clientX,
				stackWidth,
				stackInputWidth,
				INPUT_WIDTH
	...
	function handleKeyDown(event) {
		if (
			...
			arrows.selectedArrows.length > 0
		) {
	...
	return (
		...
		>
			...
			<main
				style={{
					gridTemplateColumns: `${stackWidth}px min-content auto min-content`
				}}
			>
				<Stack />
				<div
